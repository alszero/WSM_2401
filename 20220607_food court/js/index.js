    ...
    try{
      if(json['mealServiceDietInfo'][0]['head'][1]['RESULT']['CODE']=='INFO-000'){
              ...
              try{
                      ...
                      //(5.13.) 삭제하자
                      breakfastData = breakfastData.replace(/\([0-9\.]*\)/g, ""); //정규표현식: (문자 숫자나 .)문자
                      //(             \)
                      // 숫자 한글자       [0123456789]
                      //      .       \.
                      // 0~n개         *
              ...
              try{
                      ...
                      lunchData = lunchData.replace(/\([0-9\.]*\)/g, ""); //정규표현식: (문자 숫자나 .)문자
                      lunch.innerHTML = lunchData;
              }catch{
                      lunch.innerHTML = "없응"
              }
              try{
                      let dinnerData = json['mealServiceDietInfo'][1]['row'][2]['DDISH_NM']; //석식
                      dinnerData = dinnerData.replace(/\([0-9\.]*\)/g, ""); //정규표현식: (문자 숫자나 .)문자
